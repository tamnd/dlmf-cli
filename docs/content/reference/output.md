...
describes what users get. Pick a format with `-o`, or let dlmf choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dlmf <command> -o table   # aligned columns for reading
dlmf <command> -o jsonl   # one JSON object per line, for piping
dlmf <command> -o json    # a single JSON array
dlmf <command> -o csv     # spreadsheet friendly
dlmf <command> -o tsv     # tab-separated
dlmf <command> -o url     # just the URL column
dlmf <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dlmf <command> --fields id,title,url
```
...
```bash
dlmf <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dlmf <command>            # a table, because this is a terminal
dlmf <command> | wc -l    # JSONL, because this is a pipe
```
