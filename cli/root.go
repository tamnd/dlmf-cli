// Package cli builds the dlmf command tree on top of the dlmf library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: dlmf.DefaultConfig()}

	root := &cobra.Command{
		Use:   "dlmf",
		Short: "Browse NIST Digital Library of Mathematical Functions",
		Long: `dlmf reads the NIST Digital Library of Mathematical Functions
(https://dlmf.nist.gov) and returns records as table, JSON, JSONL,
CSV, TSV, or URLs. No API key required.

dlmf is an independent tool and is not affiliated with NIST.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVar(&app.cfg.BaseURL, "base-url", app.cfg.BaseURL, "DLMF base URL")
